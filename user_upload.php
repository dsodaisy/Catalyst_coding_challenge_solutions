...
elseif(!isset($arg_options['create_table']))
{
...
else //if all the parameters are entered as expected, proceed with validating the parameters/arguments provided
{
				...
				elseif($one_element!=0)
				{
					for($i=0;$i<count($all_Rows);$i++)
					{
						if($cell_value==end($all_Rows[$i]))
							array_push($rowdata, ''); //diff btw 2 NULLS????
					}
				}
			}
	...
	if(isset($arg_options['dry_run']))//if dry_run is set
	{
		...
		for($i=0;$i<count($all_Rows);$i++)//iterate through $all_Rows
		{
			...
			elseif($all_Rows[$i][2]=='')
			{
				$table->addRow(array($all_Rows[$i][0],$all_Rows[$i][1],$all_Rows[$i][3],"This email will not be entered into the DB as this email address is already assigned to another user!"));
			}
			else
			{
	...
	else//if dry_run is not set, need to create table and insert into table
	{
		...
		else //if error in dropping table 'users'
		{
			print_r($stmt->errorInfo());
			exit("Unfortunately, there was an error while attempting to create table 'users'. Please contact your administrator with the above error code/statement to help resolve this issue");
		}
	}
}
?>
